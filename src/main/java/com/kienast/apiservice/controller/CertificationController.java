	...
	public ResponseEntity<CertificationModel> addCertifaction(@Valid CertificationModel certificationModel) {
		...
		try {
			entryResponse = webClientBuilder.build()
					...
					.body(BodyInserters.fromObject(certificationModel))
					.retrieve() //run command
					.onStatus(HttpStatus::is4xxClientError, response -> {
			            return Mono.error(new BadRequestException(
			                    String.format("Failed! %s", certificationModel.getShortname())
			            ));
			        })
	...
	public ResponseEntity<CertificationModel> getCertification(String shortname) {
		...
		try {
			response = webClientBuilder.build()
					.get() //RequestMethod
					.uri(certURL+"/cert/" + shortname)
					.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
					.retrieve() //run command
					.bodyToMono(CertificationModel.class)
					.block();
		}catch(Exception e) {
			System.out.println(e.getMessage());
			e.printStackTrace();
